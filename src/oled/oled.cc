...
namespace esp_sio_dev
{
	namespace oled
	{
		...
		void EnableScroll(void *ignore)
		{
			...
			if (espRc == ESP_OK)
			{
				ESP_LOGI(kLogPrefix, "Scroll command succeeded");
			}
			else
			{
				ESP_LOGE(kLogPrefix, "Scroll command failed. code: 0x%.2X", espRc);
			}
		...
		void Init(void)
		{
			...
			if (espRc == ESP_OK)
			{
				ESP_LOGI(kLogPrefix, "OLED configured successfully");
			}
			else
			{
				ESP_LOGE(kLogPrefix, "OLED configuration failed. code: 0x%.2X", espRc);
			}
			i2c_cmd_link_delete(cmd);
